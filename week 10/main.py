...
class ProbHash:
    ...
    def hash(self, key):
        ...
        ikey = key % self.__size
        if self.__hash_table[ikey] == []:
            return int(ikey)
        else:
    ...
    def insert_data(self, student):
        ...
        if [] in self.__hash_table:
            ...
            self.__hash_table[ikey] = student
    ...
    def search_data(self, std_id):
        ...
        ikey = std_id % self.__size
        count = 0
        while self.__hash_table[ikey] != [] and self.__hash_table[ikey].get_std_id() != std_id\
        and count <= self.__size:
            ikey = self.rehash(ikey)
            count += 1
        if self.__hash_table[ikey] != [] and self.__hash_table[ikey].get_std_id() == std_id:
            print("Found %d at index %d" % (std_id, ikey))
            # if self.hash_table[ikey] is not None:
            #     self.hash_table[ikey].print_details()
            return self.__hash_table[ikey]
        else:
